...
export async function checkItemLimited(
  ...
  if (state.item.isLimited() && state.item.system.uses.value < state.data.action ? state.data.action.cost.value : state.item.system.cost.value) {
...
export async function updateItemAfterAction(
  ...
  if (state.item && limited_loading && attacks) {
    ...
    if (state.item.isLimited()) itemChanges.uses = { value: Math.max(state.item.system.uses.value - state.data.action ? state.data.action.cost.value : state.item.system.cost.value, 0) };
