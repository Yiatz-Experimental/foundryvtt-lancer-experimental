...
export async function checkItemLimited(
  ...
  if (state.item.isLimited() && state.item.system.uses.value < (state.data.action.cost ?? state.item.system.cost ?? 1)) {
...
export async function updateItemAfterAction(
  ...
  if (state.item && limited_loading && attacks) {
    ...
    if (state.item.isLimited()) itemChanges.uses = { value: Math.max(state.item.system.uses.value - (state.data.action.cost ?? state.item.system.cost ?? 1), 0) };
